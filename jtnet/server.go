...
func (s *Server) AddHandler(msgId uint32, h protocal.Handler) {
	s.msgHandler.AddHandler(msgId, h)
}
...
func (s *Server) SendBySim(sim string, id uint16, fragflag uint8, data []byte) error {
	c, err := s.ConnMgr.GetBySim(sim)
	if err != nil{
		return err
	}
	err = c.SendMsg(id, sim, fragflag, data)
	return err
}